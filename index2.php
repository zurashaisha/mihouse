...
<table border=0>
    <tbody width=100%>
        ...
        <tr>
            <td><input type=button class=button onclick="set_answer(name)" id="answer_0" name=""></td>
            <td><input type=button class=button onclick="set_answer(name)" id="answer_1" name=""></td>
        </tr>
        <tr>
            <td><input type=button class=button onclick="set_answer(name)" id="answer_2" name=""></td>
            <td><input type=button class=button onclick="set_answer(name)" id="answer_3" name=""></td>
        </tr>
    </tbody>
</table>
<?php
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (var i=0; i<answers.length; i++) {
                    ...
                    $('#answer_'+i).val(answers[i]["answer"]);
                    $('#answer_'+i).attr("name", question_index+"#"+answers[i]["index"]);
    ...
    function set_answer(name) {
        var uid=<?php echo $_SESSION["UID"]; ?>;
        $.ajax({
            url: 'process.php',
            type: 'post',
            data: {request: 'set_answer', answer_data:name, uid:uid},
            dataType: 'json',
            success: function(data) {
                if (data == "over") {
                    game_begin.style.display = 'none';
